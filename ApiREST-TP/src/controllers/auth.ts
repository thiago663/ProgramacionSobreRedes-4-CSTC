...
export const signup = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const token: string = jwt.sign({ _id: savedUser._id }, process.env['TOKEN_SECRET'] || 'LLAVE_PRIVADA', {
            expiresIn: 60 * 60 * 24
        });
...
export const signin = async (req: Request, res: Response) => {
    ...
    // Create a Token
    const token: string = jwt.sign({ _id: user._id }, process.env['TOKEN_SECRET'] || 'LLAVE_PRIVADA');
    res.header('auth-token', token).json(token);
};
